...
def try_single_key(key, user_message):
    ...
    try:
        # Set a strict 4-second timeout so a dead key won't hang the thread
        ...
        # Return the error so the main controller knows this thread failed
        return e
...
def get_ai_response(user_message):
    ...
    # Run all key requests simultaneously across parallel workers
    with ThreadPoolExecutor(max_workers=len(api_keys)) as executor:
        futures = {executor.submit(try_single_key, key, user_message): key for key in api_keys}
        
        for future in as_completed(futures):
            result = future.result()
            # If the result isn't an Exception, it means a key successfully answered!
...
# --------------------------------------------------
# FIREBASE LOGIN ROUTE (UNCHANGED)
# --------------------------------------------------
